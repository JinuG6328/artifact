...
if __name__ == "__main__":
    ...
    # With this objective, no forms are recompiled for subsequent Hessian-vector products
	#J = assemble(0.5 * inner(alpha(rho) * u, u) * dx + mu * inner(grad(u), grad(u)) * dx)
	# With this objective, forms recompile with each Hessian-vector product
	e = Expression("sin(pi * x[0]) * sin(pi * x[1])", degree = 1)
	f = interpolate(e,W.sub(1).collapse())
	J = assemble((0.5*inner(w[1], f))*dx)
	J = J*J
	m = Control(rho)
	Jhat = ReducedFunctional(J, m, eval_cb_post=eval_cb)

	a_dot1 = Function(A)
	a_dot1.vector()[:] = np.ascontiguousarray(np.random.randn(a_dot1.vector().size()))
	a_dot2 = Function(A)
	a_dot2.vector()[:] = np.ascontiguousarray(np.random.randn(a_dot2.vector().size()))
	a_dot3 = Function(A)
	a_dot3.vector()[:] = np.ascontiguousarray(np.random.randn(a_dot3.vector().size()))

	b_dot1 = Function(A)
	b_dot2 = Function(A)
	b_dot3 = Function(A)

	hello = compute_gradient(Jhat.functional, m)
	print ("Hessian-vector product 1")
	b_dot1 = Jhat.hessian(a_dot1)
	print ("Hessian-vector product 2")
	b_dot2 = Jhat.hessian(a_dot2)
	print ("Hessian-vector product 3")
	b_dot3 = Jhat.hessian(a_dot3)
