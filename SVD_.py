...
def get_matrix(A):
    ...
    q_dot = Function(fs)
    c_dot = Function(fs)
    size_of_mat = len(q_dot.vector().get_local())
    ...
    for i in range(size_of_mat):
        ...
        c_dot[i] = 1
        q_dot.vector().set_local(np.ascontiguousarray(c_dot))
        c_dot = A.hessian(q_dot)
        A_np[:,i] = c_dot.vector().get_local()    
...
def safe_sparse_dot(a, b):
    ...
    if isinstance(a, ReducedFunctional):
        ...
        for i in range(len(b.T)):
            q_dot.vector().set_local(np.ascontiguousarray(b.T[i]))
            import pdb
            pdb.set_trace()
            c[:,i] = a.hessian(q_dot).vector().get_local()
        return c
        ...
        for i in range(len(b.T)):
            q_dot.vector().set_local(np.ascontiguousarray(b.T[i]))
            c[:,i] = a.dot(q_dot).vector().get_local()
        return c
